...
my %options = (
    ## An input bam file to read
    input => 'bamfiles/10_pro_genome.bam',
    ## The text file to which to write
    outdir => '',
    ## A gff annotation
    orientation => 'start',
    sizes => "25,26,27,28,29,30,31,32,33",
    fasta => "~/scratch/libraries/genome/lmajor.fasta",
    debug => 0,
    asite => 1,
    psite => 1,
    esite => 1,
    gff => undef,
    gff_type => "gene",
    ## How far to the 5' of the start or stop codon to count
    );
my $opt = GetOptions(
    ## Input bam file to count
    "input|i:s" => \$options{input},
    ## An output directory to print chromosomes, states, and transitions
    "outdir|o:s" => \$options{outdir},
    ## Orientation (start/end) to count reads from
    "orientation:s" => \$options{orientation},
    ## A list of acceptable read sizes
    "sizes|s:s" => \$options{sizes},
    ## The fasta genome for counting against
    "fasta|f:s" => \$options{fasta},
    ## Stop early and print debugging information
    "debug:s" => \$options{debug},
    ## Include state changes for the codon under the: A,P,E site
    "asite|a:s" => \$options{asite},
    "psite|p:s" => \$options{psite},
    "esite|e:s" => \$options{esite},
    ## If gff gets defined, include a htseq count table
    "gff|g:s" => \$options{gff},
    ## Define the type of hit to include for htseq-like output
    "gff_type:s" => \$options{gff_type},
    );
...
sub Graph_Reads {
    ...
    my %options = (
    ## The text output from count_reads.pl
    input => 'procyclic_counts.txt',
    ## The directory into which to put the output files.
    output => 'graph_counts/',
    ## The minimum size of RPFs to count
    minsize => 24,
    ## The maximum size of RPFs to count
    maxsize => 36,
    ## The earliest position with respect to the start codon to count
    minpos => -30,
    ## The latest position with respect to the start codon to count
    maxpos => 30,
    ## Perform read position correction based on previous analyses?
    correction => 1,
    ## Orientation refers to whether we want to observe the start or end of ORFs, assume the start
    orientation => 'start',
    ## Anchor defines whether we count from the start or end of reads
    anchor => 'start',
    help => undef,
    );
my $opt = GetOptions(
    ...
    "minsize:f" => \$options{minsize},
    "maxsize:s" => \$options{maxsize},
    "maxpos|m:s" => \$options{maxpos},
    "minpos:s" => \$options{minpos},
    "correction|c:s" => \$options{correction},
    "orientation:s" => \$options{orientation},
