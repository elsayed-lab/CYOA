...
my $cyoa = new CYOA(input => qq"t/data/test_forward.fastq.gz", pbs => 0);
diag("Does fastqc execute?");
ok($cyoa->Fastqc_Single(),     'Run Fastqc');
diag("Can I collect fastqc statistics into fastqc_stats.csv?");
ok(my $fastq_result = $cyoa->Last_Stat(input => 'outputs/fastqc_stats.csv'),      'Collect Fastqc Statistics');
