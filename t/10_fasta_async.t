...
## Note, using Test::File::ShareDir::Dist has an important implication:
## The input file(s) provided by T:F::ShareDir are sent to a temporary
## directory which is not available to the compute nodes, so before
## making use of the share files, they must be copied out to the local directory.
